unit rr4dl.Request.Client;

interface

uses rr4dl.Request.Contract, Data.DB, REST.Client, REST.Response.Adapter,
  REST.Types, System.SysUtils, System.Classes,
  rr4dl.Response.Contract, j4dl, //System.JSON,
...
type
  ...
  private
    ...
    function AddBody(const AContent: string;
      ...
      : IRequest; overload;
    function AddBody(const AContent: TJSONObject; const AOwns: Boolean = False)
      : IRequest; overload;
    function AddBody(const AContent: TJSONArray; const AOwns: Boolean = False)
      : IRequest; overload;
    function AddBody(const AContent: TStream; const AOwns: Boolean = False)
      : IRequest; overload;
    function FallbackCharsetEncoding(const AFallbackCharsetEncoding: string)
      : IRequest;
...
uses System.Generics.Collections, rr4dl.Response.Client;
...
begin
  ...
  if not Assigned(AContent) then
    Exit;
{$IF COMPILERVERSION <= 29}
  FRESTRequest.AddBody(AContent.Stringify);
{$ELSE}
  FRESTRequest.Body.Add(AContent.Stringify);
{$ENDIF}
  if AOwns then
    AContent.Free;
end;
...
begin
  ...
  if not Assigned(AContent) then
    Exit;
  Self.AddBody(AContent.Stringify);
  if AOwns then
    AContent.Free;
end;
...
function TRequestClient.FallbackCharsetEncoding(const AFallbackCharsetEncoding
  : string): IRequest;
begin
  Result := Self;
